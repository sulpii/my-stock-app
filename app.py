...
# 1. 페이지 레이아웃 설정
st.set_page_config(
    page_title="Wharton Portfolio Analytics Terminal", 
    page_icon="🏛️", 
    layout="wide"
)
...
# 2. 헤더
st.markdown('<p class="main-header">🏛️ Wharton Investment Competition Terminal</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-header">글로벌 11대 GICS 산업 섹터 & 벤치마크(S&P500) 대비 알파(Alpha) 분석 단말기</p>', unsafe_allow_html=True)
...
if selected_tickers:
    ...
    if valid_tickers:
        ...
        with tab1:
            ...
            st.caption("시장 대표 지수(SPY) 대비 내가 선택한 주식들의 상대적 성과 흐름입니다.")
        ...
        # TAB 2: 포트폴리오 백테스터 & Alpha
        with tab2:
            st.markdown("### 🎯 포트폴리오 비중 설정 및 초과수익(Alpha) 분석")
            ...
            if total_weight == 0:
                ...
                m2.metric("시장 초과 수익 (Alpha)", f"{alpha:+.2f}%", delta=f"{alpha:.2f}%p")
...
**2. 성과 및 리스크 측정 (Performance & Benchmark)**
